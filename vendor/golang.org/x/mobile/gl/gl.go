...
func (ctx *context) CreateTexture() Texture {
	return Texture(uint32(ctx.enqueue(call{
		args: fnargs{
			fn: glfnGenTexture,
		},
		blocking: true,
...
func (ctx *context) GetAttribLocation(p Program, name string) Attrib {
	s, free := ctx.cString(name)
	defer free()
	return Attrib(uint(ctx.enqueue(call{
		args: fnargs{
...
func (ctx context3) DrawArraysInstanced(mode Enum, v0, v1, v2 int) {
	ctx.enqueue(call{
		args: fnargs{
...
func (ctx context3) DrawElementsInstanced(mode Enum, v0 int, v1 Enum, v2, v3 int) {
	ctx.enqueue(call{
		args: fnargs{
...
func (ctx context3) VertexAttribDivisor(index Attrib, v0 int) {
	ctx.enqueue(call{
		args: fnargs{
