...
#ifndef HAS_SINGLE_INSTANCE
	mat4 u_ModelMatrix = a_Transform;
	mat4 u_NormalMatrix = a_NormalTransform;
#endif
...
// Why? gles3 doesn't support fetching depth by dynamic index
// so we do some hackering to fetch a depth with a const index by
// branching and whatnots, it's probably slow but works so far
...
// Unlight this func by passing range and samplerCube
float shadowCubeCalculation(Light light, vec3 fragPos, vec3 pointToLight, float dist) {
...
// Unlight this func by passing Matrix, and samplerCube
float shadow2DCalculation(Light light, vec3 fragPos) {
	int di = light.depthIndex;
	// This can be done at vertex shader?
...
vec4 getBaseColor() {
...
#if defined(MATERIAL_UNLIT) && defined(HAS_BASE_COLOR_MAP)
	baseColor *= texture(u_BaseColorSampler, getBaseColorUV());
	// New multiply baseColorFactor to control color with a Vec4
	...
	return baseColor * getVertexColor() * v_InstanceColor;
}
...
MaterialInfo getMetallicRoughnessInfo(MaterialInfo info, float f0_ior) {
	...
	vec4 mSample = texture(u_MetallicSampler, getMetallicRoughnessUV());
	info.metallic *= mSample.r;
#endif
#ifdef HAS_ROUGHNESS_MAP
	vec4 rSample = texture(u_RoughnessSampler, getMetallicRoughnessUV());
...
void main() {
...
#ifdef USE_PUNCTUAL
	int maxLights = min(u_nLights, LIGHT_COUNT);
	for (int i = 0; i < maxLights; ++i) {
		...
		if (NdotL > 0.0 || NdotV > 0.0) {
			...
			// Calculate gorge Shadow here?
			float f_shadow = ShadowCalculation(light, v_Position, pointToLight, length(pointToLight));
			intensity *= (1.0 - f_shadow);
	...
	// ALPHA_PREMUL
	g_finalColor = vec4(toneMap(color)*baseColor.a, baseColor.a);
