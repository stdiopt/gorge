...
func ColorPicker(fn func(m32.Vec4)) Func {
	...
	return func(b *Builder) {
		...
		pickColor := func(pos m32.Vec2) {
			...
			if res.Hit {
				wp := picker.WorldPosition()
				x := int(((res.Position[0] - wp[0]) / rect[2]) * float32(colorTex.Width))
				// y is scaled y -1
				y := int((1 - ((wp[1] - res.Position[1]) / rect[3])) * float32(colorTex.Height))
		...
		gorge.HandleFunc(picker, func(e gorgeui.EventPointerDown) { pickColor(e.Position) })
		gorge.HandleFunc(picker, func(e gorgeui.EventDragBegin) { pickColor(e.Position) })
		gorge.HandleFunc(picker, func(e gorgeui.EventDrag) { pickColor(e.Position) })
