//go:build !js && !android && !mobile && !gorgl

// Package gl makes gl implementations uniform
// nolint: lll,golint
package gl
...
func (g *Wrapper) FramebufferRenderbuffer(target Enum, attachment Enum, rbTarget Enum, rb Renderbuffer) {
	gl.FramebufferRenderbuffer(target, attachment, rbTarget, rb)
}
...
func (g *Wrapper) GetBufferParameteri(target Enum, value Enum) int {
	var bi int32
	gl.GetBufferParameteriv(target, value, &bi)
	return int(bi)
}
...
func (g *Wrapper) GetProgramInfoLog(p Program) string {
	logLen := g.GetProgrami(p, gl.INFO_LOG_LENGTH)

	log := strings.Repeat("\x00", logLen+1)
	gl.GetProgramInfoLog(p, int32(logLen), nil, gl.Str(log))
	return log
}
...
func (g *Wrapper) RenderbufferStorage(target Enum, internalFormat Enum, width int, height int) {
	gl.RenderbufferStorage(target, internalFormat, int32(width), int32(height))
}
...
func (g *Wrapper) StencilFunc(fn Enum, ref int, mask uint32) {
	gl.StencilFunc(fn, int32(ref), mask)
}
...
func (g *Wrapper) Uniform4fv(dst Uniform, src []float32) {
	gl.Uniform4fv(int32(dst), 1, &src[0])
}
...
func (g *Wrapper) GetUniformBlockIndex(p Program, name string) uint32 {
	return gl.GetUniformBlockIndex(p, gl.Str(name+"\x00"))
}
...
func (g *Wrapper) UniformBlockBinding(p Program, index uint32, bind Enum) {
	gl.UniformBlockBinding(p, index, bind)
}
...
func (g *Wrapper) GetActiveUniformBlockName(p Program, index uint32) string {
	...
	gl.GetActiveUniformBlockName(p, index, nameSz, &sz, &nameBuf[0])

	return gl.GoStr(&nameBuf[0])
}
